...
PORT = 8081  # Use 8081 to avoid conflict with Docker nginx on 8080
DIRECTORY = os.path.dirname(os.path.abspath(__file__))

# Default to production testnet, override with env var
RPC_URL = os.environ.get("RPC_URL", "http://51.79.26.123:9944")
FAUCET_URL = os.environ.get("FAUCET_URL", "http://51.79.26.123:8080")
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        # Proxy faucet requests
        if self.path.startswith("/faucet/"):
            faucet_path = self.path[7:]  # Remove /faucet prefix
            try:
                req = urllib.request.Request(
                    f"{FAUCET_URL}{faucet_path}",
